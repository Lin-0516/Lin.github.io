...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clickNumber"  in storeup.getallcolumn(storeup,storeup):
...
def storeup_save(request):
    ...
    后台新增
    '''
    request.funname = __name__+"."+storeup_save.__name__
    request.operation = "新增我的收藏"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        ...
        idOrErr= storeup.createbyreq(storeup,storeup, req_dict,"否")
        ...
        error= storeup.createbyreq(storeup,storeup, req_dict,"否")
...
def storeup_add(request):
    ...
    前台新增
    '''
    request.funname = __name__+"."+storeup_add.__name__
    request.operation = "新增我的收藏"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def storeup_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clickNumber" in storeup.getallcolumn(storeup,storeup):
            try:
...
def storeup_update(request):
    '''
    '''
    request.funname = __name__+"."+storeup_update.__name__
    request.operation = "更新我的收藏"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def storeup_delete(request):
    ...
    批量删除
    '''
    request.funname = __name__+"."+storeup_delete.__name__
    request.operation = "删除我的收藏"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def storeup_importExcel(request):
    request.funname = __name__+"."+storeup_importExcel.__name__
    request.operation = "导入我的收藏"
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    storeup.createbyreq(storeup, storeup, req_dict,"否")
...
def storeup_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(user_id)+" and tablename = 'storeup' order by addtime desc")
