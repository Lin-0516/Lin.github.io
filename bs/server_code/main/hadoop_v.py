...
hadoop_path="D:/clhadoop/single/hadoop-3.4.1"
# 连接到 MySQL 数据库
...
def upload_csv_mapreduce_hadoop():
    # 查询数据
    query = "SELECT * FROM cnhnbsg"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"cnhnbsg.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/cnhnbsg.csv'
    # 目标 HDFS 路径
...
def send_cmd():
    ...
    [
        ...
        "-mapper", f"\"python value_mapper.py {csv_index('cnhnbsg.csv','title')} {csv_index('cnhnbsg.csv','jiage')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/cnhnbsg.csv",
        "-output", "hdfs://localhost:9000/output/cnhnbsg/valuetitlejiage"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('cnhnbsg.csv','title')} {csv_index('cnhnbsg.csv','xunjia')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/cnhnbsg.csv",
        "-output", "hdfs://localhost:9000/output/cnhnbsg/valuetitlexunjia"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('cnhnbsg.csv','qipi')}\"",
        "-reducer", "\"python group_reducer.py qipi\"",
        "-input", "hdfs://localhost:9000/input/cnhnbsg.csv",
        "-output", "hdfs://localhost:9000/output/cnhnbsg/groupqipi"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('cnhnbsg.csv','leibie')}\"",
        "-reducer", "\"python group_reducer.py leibie\"",
        "-input", "hdfs://localhost:9000/input/cnhnbsg.csv",
        "-output", "hdfs://localhost:9000/output/cnhnbsg/groupleibie"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('cnhnbsg.csv','tags')}\"",
        "-reducer", "\"python group_reducer.py tags\"",
        "-input", "hdfs://localhost:9000/input/cnhnbsg.csv",
        "-output", "hdfs://localhost:9000/output/cnhnbsg/grouptags"
    ],
    ]
