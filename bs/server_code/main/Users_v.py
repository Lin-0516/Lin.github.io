...
def users_register(request):
    if request.method in ["POST", "GET"]:
        ...
        error = users.createbyreq(users, users, req_dict,"是")
...
def users_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= users.__loginUserColumn__
        except:
        ...
        records=users.getbyparams(users, users, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg, encoder=CustomJsonEncoder)

        eval('''users.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clickNumber"  in users.getallcolumn(users,users):
...
def users_query(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = users.objects.filter(**request.session.get("req_dict")).values()
...
def users_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __foreEndList__=users.__foreEndList__
        except:
            __foreEndList__=None

        #forrEndListAuth
        try:
            __foreEndListAuth__=users.__foreEndListAuth__
        except:
        ...
        #authSeparate
        try:
            __authSeparate__=users.__authSeparate__
        except:
        ...
        try:
            __authTables__=users.__authTables__
        except:
            __authTables__=None
...
def users_save(request):
    ...
    后台新增
    '''
    request.funname = __name__+"."+users_save.__name__
    request.operation = "新增管理员"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        ...
        idOrErr= users.createbyreq(users,users, req_dict,"是")
...
def users_add(request):
    ...
    前台新增
    '''
    request.funname = __name__+"."+users_add.__name__
    request.operation = "新增管理员"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def users_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clickNumber" in users.getallcolumn(users,users):
            try:
...
def users_update(request):
    '''
    '''
    request.funname = __name__+"."+users_update.__name__
    request.operation = "更新管理员"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def users_delete(request):
    ...
    批量删除
    '''
    request.funname = __name__+"."+users_delete.__name__
    request.operation = "删除管理员"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def users_importExcel(request):
    request.funname = __name__+"."+users_importExcel.__name__
    request.operation = "导入管理员"
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    users.createbyreq(users, users, req_dict,"是")
...
def users_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(user_id)+" and tablename = 'users' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from users where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from users where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
