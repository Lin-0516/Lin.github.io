...
class BaseModel(models.Model):
    ...
    def __Page(self, model, params, request, q):
        '''
        刷表专用
        http://ip:port/Cl682072280/${tableName}/page
        page 当前页
        pagesize 每页记录的长度
        ...
        limit = copy.deepcopy(params.get('limit')) if params.get('limit') != None else 999999999
        try:
            del params['sort']
        except:
            pass
                ...
                if fuzzy_key != None:
                    ...
                    if length >= 2:
                        ...
                        for i, v in enumerate(valList):
                            if i == length - 1:
                                filterStr += 'Q({}__icontains="{}")'.format(fuzzy_key, v.replace("%", ""))
                            else:
                                filterStr += 'Q({}__icontains="{}")|'.format(fuzzy_key, v.replace("%", ""))
                        contain_str += '.filter({})'.format(filterStr)
                    else:
        ...
        modelobj = apps.get_model('main', model.__doc__)
    ...
    def __CreateByReq(self, model, params,authPeople):
        '''
        根据请求参数创建对应模型记录的公共方法
        :param model:
        :param params:
    ...
    def createbyreq(self, model, params,authPeople):
        '''
        根据请求参数创建对应模型记录
        :param model:
        :param params:
        :return:
        '''
        return self.__CreateByReq(model, model, params,authPeople)
