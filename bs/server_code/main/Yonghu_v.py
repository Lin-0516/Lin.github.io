...
def yonghu_register(request):
    if request.method in ["POST", "GET"]:
        ...
        error = yonghu.createbyreq(yonghu, yonghu, req_dict,"是")
...
def yonghu_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        recordsParam = {}
        recordsParam[username_str] = req_dict.get("zhanghao")
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clickNumber"  in yonghu.getallcolumn(yonghu,yonghu):
...
def yonghu_save(request):
    ...
    后台新增
    '''
    request.funname = __name__+"."+yonghu_save.__name__
    ...
    if request.method in ["POST", "GET"]:
        ...
        if yonghu.objects.filter(zhanghao = req_dict['zhanghao']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "账号已存在"
            return JsonResponse(msg, encoder=CustomJsonEncoder)
        req_dict['mima'] = Common.md5(Common, req_dict.get('mima'))
        ...
        idOrErr= yonghu.createbyreq(yonghu,yonghu, req_dict,"是")
...
def yonghu_add(request):
    ...
    前台新增
    '''
    request.funname = __name__+"."+yonghu_add.__name__
    request.operation = "新增用户"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def yonghu_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clickNumber" in yonghu.getallcolumn(yonghu,yonghu):
            try:
...
def yonghu_update(request):
    '''
    '''
    request.funname = __name__+"."+yonghu_update.__name__
    ...
    if request.method in ["POST", "GET"]:
        ...
        if len(old_detail)>0:
            ...
            if req_dict.get('mima') != None and req_dict.get('mima') != old_detail.get('mima'):
                req_dict['mima'] = Common.md5(Common, req_dict.get('mima'))
...
def yonghu_delete(request):
    ...
    批量删除
    '''
    request.funname = __name__+"."+yonghu_delete.__name__
...
def yonghu_importExcel(request):
    request.funname = __name__+"."+yonghu_importExcel.__name__
    request.operation = "导入用户"
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    yonghu.createbyreq(yonghu, yonghu, req_dict,"是")
...
def yonghu_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(user_id)+" and tablename = 'yonghu' order by addtime desc")
