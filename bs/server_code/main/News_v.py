...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clickNumber"  in news.getallcolumn(news,news):
...
def news_save(request):
    ...
    后台新增
    '''
    request.funname = __name__+"."+news_save.__name__
    request.operation = "新增公告资讯"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        ...
        idOrErr= news.createbyreq(news,news, req_dict,"否")
        ...
        error= news.createbyreq(news,news, req_dict,"否")
...
def news_add(request):
    ...
    前台新增
    '''
    request.funname = __name__+"."+news_add.__name__
    request.operation = "新增公告资讯"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def news_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clickNumber" in news.getallcolumn(news,news):
            try:
...
def news_update(request):
    '''
    '''
    request.funname = __name__+"."+news_update.__name__
    request.operation = "更新公告资讯"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def news_delete(request):
    ...
    批量删除
    '''
    request.funname = __name__+"."+news_delete.__name__
    request.operation = "删除公告资讯"
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
...
def news_importExcel(request):
    request.funname = __name__+"."+news_importExcel.__name__
    request.operation = "导入公告资讯"
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    news.createbyreq(news, news, req_dict,"否")
...
def news_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(user_id)+" and tablename = 'news' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from news where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from news where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
