...
def schemaName_file_upload(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if file:
            ...
            filePath = os.path.join(os.getcwd(), "templates/file", file_name)
...
def schemaName_upload_forecast(request,tableName,fileName):
    ...
    if request.method in ["POST", "GET"]:
        fullPath = request.get_full_path()
        print("{}=============>".format(sys._getframe().f_code.co_name), fullPath)
        path1 = os.path.join(os.getcwd(), "templates/file/", tableName,fileName)
...
def schemaName_file_download(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        filePath = os.path.join(os.getcwd(), "templates/file", filename)
        print("filePath===========>", filePath)
...
def schemaName_matchface(request):
    ...
    if request.method in ["POST", "GET"]:
        try:
            ...
            file_path1 = os.path.join(os.getcwd(),"templates/file",face1)

            face2 = req_dict.get("face2")
            file_path2 = os.path.join(os.getcwd(), "templates/file", face2)
...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        # Linux
        # cmd = "cd /yykj/python/9999/s8118182 && scrapy crawl "+tableName+" -a databaseName=cl682072280 -a realtime=False"
        # Windows
        cmd = "cd E:/bishe/bs/spider_code && scrapy crawl " + tableName +" -a databaseName=cl682072280 -a realtime=False"
        os.system(cmd)

        return JsonResponse(msg)

# 数据备份
def schemaName_mysqldump(request):
    if request.method in ["POST", "GET"]:

        filePath = os.path.join(os.getcwd(), "templates/file", "backup.sql")
        cmd = "/usr/bin/mysqldump -h127.0.0.1 -uroot -P3306 -p123456 cl682072280 > " + filePath
        os.system(cmd)
...
def schemaName_encrypt(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{}}
        msg["data"] = Common.md5(Common,request.session.get("req_dict").get("text"))
        return JsonResponse(msg)
...
def schemaName_file_encrypt(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        input_path = os.path.join(os.getcwd(), "templates/file", filename)
        output_path = os.path.join(os.getcwd(), "templates/file")
        if not os.path.exists(input_path):
            os.makedirs(output_path)
        aes_encryptor = FileEncryptor("1234567890123456!", algorithm=type)
