...
class Xauth(MiddlewareMixin):
    def process_request(self,request):
        ...
        if request.method == 'GET':
            ...
            if fullPath not in post_list and "register" not in fullPath and "login" not in fullPath and "faceLogin" not in fullPath and "update" not in fullPath:  # 注册时不检测token。
                result = Auth.identify(Auth, request)
